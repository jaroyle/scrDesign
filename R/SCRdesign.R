...
SCRdesign <- function(statespace=NULL, all.traps=NULL, 
  ...
  for(m in 1:ndesigns){
    ...
    Q <- Qfn(X=X, S=statespace, N=N, sigma=sigma, beta0=beta0, ntraps=ntraps)[crit]     # evaluate Qfn|X
      ...
      for(i in 1:ntraps){                      # here we will replace each site with [nn] alternatives
        ...
        if(length(x.consider)>0){
          for(j in 1:length(x.consider)){
            ...
            getQs <- Qfn(X=Xtest,S=statespace,N=N,sigma=sigma,beta0=beta0,ntraps=ntraps)
            qtest[j] <- getQs[crit]
          }
        }else{
          qtest <- NaN
